// Package wim implements a WIM file parser and splitter.
//
...
//
// The reader implementation is based on github.com/Microsoft/go-winio/wim (MIT License),
// modified to remove build constraints for cross-platform support.
package wim
...
// ResourceDescriptor describes a resource (blob) in a WIM file.
...
// Flags returns the resource flags.
func (r *ResourceDescriptor) Flags() resFlag {
	return resFlag(r.FlagsAndCompressedSize >> 56)
}

// CompressedSize returns the compressed size of the resource.
...
// StreamDescriptor describes a stream (blob with hash) in a WIM file.
...
const supportedHdrFlags = hdrFlagRpFix | hdrFlagReadOnly | hdrFlagCompressed | hdrFlagCompressLzx | hdrFlagSpanned

// WimHeader is the on-disk WIM file header.
...
// Header returns the WIM file header.
func (r *Reader) Header() WimHeader {
	return r.hdr
}

// Streams returns all stream descriptors (blobs) in the WIM.
func (r *Reader) Streams() map[SHA1Hash]ResourceDescriptor {
	return r.fileData
}
...
func NewReader(f io.ReaderAt) (*Reader, error) {
	...
	// Note: we now allow opening individual parts of split WIMs for verification.
	// Full multi-part WIM reading (combining all parts) is not supported.
